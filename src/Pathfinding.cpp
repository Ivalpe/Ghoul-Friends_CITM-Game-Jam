...
void Pathfinding::DrawPath() {
	...
	while (!frontierCopy.empty()) {
		...
		Engine::GetInstance().render.get()->DrawTexture(pathTex, pos.getX(), pos.getY(), SDL_FLIP_NONE, &rect);
	...
	for (const auto& pathTile : pathTiles) {
		...
		Engine::GetInstance().render.get()->DrawTexture(tileX, pathTileWorld.getX(), pathTileWorld.getY(), SDL_FLIP_NONE);
	}
